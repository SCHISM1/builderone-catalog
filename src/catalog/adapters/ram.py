"""RAM source adapter — raw text → LLM extraction → llm-extracted observations."""
...
class RAMExtractionResult(BaseModel):
    ...
    ddr_generation: Literal["DDR4", "DDR5"] = Field(description="Memory standard: DDR4 or DDR5.")
    speed_mhz: int = Field(description="Rated speed in MHz (or MT/s), e.g. 6000.", ge=1)
    module_count: int = Field(description="Number of modules in the kit, e.g. 2.", ge=1)
    capacity_gb: int = Field(description="Total kit capacity in GB, e.g. 32.", ge=1)
...
class RAMAdapter:
    """Reads raw RAM spec text → schema-enforced LLM extraction → observations.
    ...
    @classmethod
    def from_text_file(cls, path: str, extractor: LLMExtractor) -> RAMAdapter:
    ...
    def _extract_with_retry(self, text: str) -> RAMExtractionResult | None:
        try:
            return self._extractor.extract(text, RAMExtractionResult, max_retries=1)  # type: ignore[no-any-return]
        except Exception:
            pass
        try:
            return self._extractor.extract(text, RAMExtractionResult, max_retries=1)  # type: ignore[no-any-return]
...
def _first_line(text: str) -> str:
    ...
    return lines[0] if lines else "Unknown RAM"
