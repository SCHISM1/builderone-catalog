"""PSU source adapter — raw text → LLM extraction → llm-extracted observations."""
...
class PSUExtractionResult(BaseModel):
    ...
    wattage: int = Field(description="Rated output power in watts, e.g. 850.", ge=1)
...
class PSUAdapter:
    """Reads raw PSU spec text → schema-enforced LLM extraction → observations.
    ...
    @classmethod
    def from_text_file(cls, path: str, extractor: LLMExtractor) -> PSUAdapter:
    ...
    def _extract_with_retry(self, text: str) -> PSUExtractionResult | None:
        try:
            return self._extractor.extract(text, PSUExtractionResult, max_retries=1)  # type: ignore[no-any-return]
        except Exception:
            pass
        try:
            return self._extractor.extract(text, PSUExtractionResult, max_retries=1)  # type: ignore[no-any-return]
...
def _first_line(text: str) -> str:
    ...
    return lines[0] if lines else "Unknown PSU"
